...
def save_training_log(log_file, log_content):
...
class ETTrainer:
    ...
    def __init__(self, args, hyper_params):
        ...
        self.reset_metric()
        self.metric = {'ADE': [], 'FDE': [], 'ADE_test': [], 'FDE_test': [], 'TCC_test': [], 'COL_test': []}
    ...
    def fit(self):
        ...
        for epoch in range(self.hyper_params.num_epochs):
            log_file = "./checkpoints/" + self.args.tag + "/" + self.hyper_params.dataset + "/" + "training_log.txt"
            self.train(epoch)
            self.valid(epoch)
            res = self.val1()
            self.metric['ADE'].append(res['ADE'])
            self.metric['FDE'].append(res['FDE'])
            res1 = self.test()
            self.metric['ADE_test'].append(res1['ADE'])
            self.metric['FDE_test'].append(res1['FDE'])
            self.metric['TCC_test'].append(res1['TCC'])
            self.metric['COL_test'].append(res1['COL'])
            save_training_log(log_file, str(epoch))
            save_training_log(log_file, str(res1))
            save_training_log(log_file, ("Train_loss: {0:.8f}, Val_los: {1:.8f}"
                                         .format(self.log['train_loss'][-1], self.log['val_loss'][-1])))
            save_training_log(log_file, ("Min_val_epoch: {0}, Min_val_loss: {1:.8f}"
                                         .format(np.array(self.log['val_loss']).argmin(),
            ...
            #     self.save_model()
            if epoch == 0 or self.metric['ADE'][-1] < min(self.metric['ADE'][:-1]):
                self.save_model()

            if epoch == 0 or self.metric['ADE_test'][-1] < min(self.metric['ADE_test'][:-1]):
                self.save_model(filename='model_best_test.pth')

            if epoch == 0 or self.metric['FDE'][-1] < min(self.metric['FDE'][:-1]):
                self.save_model(filename='model_best_fde.pth')

            if epoch == 0 or self.metric['FDE_test'][-1] < min(self.metric['FDE_test'][:-1]):
                self.save_model(filename='model_best_test_fde.pth')

            if epoch == 0 or self.metric['TCC_test'][-1] > max(self.metric['TCC_test'][:-1]):
                self.save_model(filename='model_best_test_tcc.pth')

            if epoch == 0 or self.metric['COL_test'][-1] < min(self.metric['COL_test'][:-1]):
                self.save_model(filename='model_best_test_col.pth')
...
class ETSequencedMiniBatchTrainer(ETTrainer):
    ...
    def train(self, epoch):
        ...
        for cnt, batch in enumerate(tqdm(self.loader_train, desc=f'Train Epoch {epoch}', mininterval=1)):
            ...
            if self.hyper_params.use_rnn:
                log_var, mu = output["encoder_traj"][:, :, 0], output["encoder_traj"][:, :, 1]
                vae_loss = torch.mean(0.5 * (log_var.exp() + torch.square(mu) - log_var).sum(dim=1))
                v1 = self.hyper_params.v1
                loss = loss + vae_loss * v1
                v2 = self.hyper_params.v2
                loss_v = output["loss_euclidean_ade_v"] + output["loss_euclidean_fde_v"]
                v3 = self.hyper_params.v3
                loss_h = (output["gt_error"] - output["head_weights"]).norm(p=2, dim=-1).mean()
                loss = loss + loss_v * v2 + loss_h * v3
    ...
    def test(self):
        ...
        for batch in tqdm(self.loader_test, desc=f"Test {self.hyper_params.dataset.upper()} scene"):
            ...
            if obs_traj.shape[0]==5:
                obs_traj1= obs_traj.cpu().numpy()
                pred_traj1 = pred_traj.cpu().numpy()
                np.save('obs_traj65u'+str(aa)+'.npy',obs_traj1)
                np.save('pred_traj65u'+str(aa)+'.npy', pred_traj1)
                np.save('out_traj65u' + str(aa) + '.npy', output["recon_traj"].cpu().numpy())
    ...
    def val1(self):
        ...
        for batch in tqdm(self.loader_val, desc=f"val {self.hyper_params.dataset.upper()} scene"):
            obs_traj, pred_traj = [tensor.cuda(non_blocking=True) for tensor in batch[:2]]
...
class ETCollatedMiniBatchTrainer(ETTrainer):
    ...
    def val1(self):
        ...
        for batch in tqdm(self.loader_val, desc=f"Test {self.hyper_params.dataset.upper()} scene"):
            obs_traj, pred_traj = [tensor.cuda(non_blocking=True) for tensor in batch[:2]]
