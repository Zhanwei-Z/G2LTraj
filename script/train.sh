...
# Arguments
while getopts t:b:c:p:d:i:s:v: flag
do
  case "${flag}" in
    t) tag=${OPTARG};;
    b) baseline=${OPTARG};;
    ...
    s) static_dist=(${OPTARG});;
    v) v2=(${OPTARG});;
    *) echo "usage: $0 [-t TAG1] [-b BASELINE] [-c CONFIG_PATH] [-d \"eth hotel univ zara1 zara2\"] [-i \"0 1 2 3 4\"]" >&2
      exit 1 ;;
  esac
done
...
do
  ...
  --v2 "${v2}"  --static_dist "${static_dist}"  --tag "${tag}" --gpu_id ${device_id_array[$i]} &
  pid_array[$i]=$!
...
for (( i=0; i<${#dataset_array[@]}; i++ ))
do
  printf "test ${dataset_array[$i]}"
  python trainval.py --test --cfg "${tag}/""${dataset_array[$i]}"/hyper_params.json \
